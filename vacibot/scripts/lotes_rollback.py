...
with open('vacibot/scripts/lotes_desativados.csv') as csv_file:
    ...
    for row in csv_reader:        
        try:
            ...
            with open("vacibot/scripts/lotes_reativados.csv", "a") as fp:
                fp.write(f"{row[0]},{row[1]},{datetime.now()}\n")
        except:
            print("Lote NÃO habilitado no Banco de Dados: ", row[0], row[1])
            #salva lotes p/ consulta
            with open("vacibot/scripts/lotes_nao_reativados.csv", "a") as fp:
                fp.write(f"{row[0]},{row[1]},{datetime.now()}\n")        
